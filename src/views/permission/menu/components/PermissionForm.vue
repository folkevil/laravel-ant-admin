<template>
  <a-modal
    ...
    <a-form :form="form" layout="inline">
      <a-row :gutter="0">
        ...
        <a-col :span="24" v-show="form.getFieldValue('pid')">
          <a-col :span="12">
            <a-form-item v-if="form.getFieldValue('type') !== 'action'" label="组件地址" hasFeedback>
              <a-select
                style="width: 184px"
                v-decorator="[
                  'component',
        ...
        <a-col :span="24">
          <a-form-item v-show="form.getFieldValue('type') === 'path'" label="Redirect" hasFeedback>
            <a-input
              v-decorator="[
                'redirect',
                {
       ...
       <a-col :span="12" v-if="form.getFieldValue('type') !== 'action'" >
          <a-form-item label="菜单权重">
            <a-input-number
              v-decorator="[
                'weight',
                {
                    rules: [{ required: true, message: '请输入菜单权重!' }],
                    initialValue: 0
                }
              ]"
            >
    ...
    >
        <template>
            <icon-selector v-model="iconSelector" @change="handleIconChange" style="margin-top: 20px"/>
        </template>
    </a-modal>
  </a-modal>
</template>
